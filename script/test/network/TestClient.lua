...
test_start_client({threadcount=10});
-----------------------------------------------
]]
...
function test_start_client(input)
	...
	for i=1, nCount do
		...
		for k=1, kSize do
			while( NPL.activate(string.format("(%s)%s:script/test/network/TestServer.lua", rts_name, server_address.nid), {TestCase = "TP", data="from client", rts_name=rts_name, }) ~=0 ) do end
		end	
	end
end
	...
	if(msg.TestCase == "TP") then	
		if(TickStat(LAPC_stats)) then
			...
			for i=1,echoes_count do 
				local res = NPL.activate(string.format("(%s)%s:script/test/network/TestServer.lua", msg.rts_name or "p1", msg.nid or msg.tid), {TestCase = "TP", rts_name = msg.rts_name, data=msg.data})
				if(res ~= 0 )then 
					break;
				end
