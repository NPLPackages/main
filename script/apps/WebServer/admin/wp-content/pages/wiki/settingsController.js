...
.controller('settingsController', function ($scope, $http, Account) {
	...
	$scope.changePassword = function (oldpassword, newpassword) {
	    $http.post("/ajax/wiki/auth/api_changepw", { oldpassword: oldpassword, newpassword: newpassword, })
	...
	$scope.updateProfile = function () {
	    if ($scope.user && $scope.user.displayName) {
	        $http.put("/ajax/wiki/auth/api_me", $scope.user)
                .then(function (response) {
                    if (response.data) {
	...
	$scope.deleteAccount = function () {
	    ...
	    {
	        $http.post("/ajax/wiki/auth/api_deleteme", {})
            .then(function (response) {
                if (response.data) {
                    alert("用户已经删除!");
                }
