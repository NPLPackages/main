...
See HTTPDebugger for example. simply extend the virtual functions and call
`SetDebugger(HTTPDebugger);` to set current debugger. 
...
function IPCDebugger.WriteDebugOutput(msg)
	...
	IPCDebugger.GetDebugger().Write({filename="DebuggerOutput", type=debug_events_enum.DEBUG_OUTPUT, code = msg});
end
local write = IPCDebugger.WriteDebugOutput
...
function IPCDebugger.Dump(...)
	...
	if(msg) then
		if(debug_debugger) then log(msg); end
		IPCDebugger.GetDebugger().Write({filename="ExpValue", type=debug_events_enum.DEBUG_OUTPUT, code = msg});
	end	
end
...
function IPCDebugger.WriteOutput(msg)
	IPCDebugger.GetDebugger().Write({filename="Output", type=debug_events_enum.DEBUG_OUTPUT, code = msg});
end

-- send a break point event to the debugger UI. 
function IPCDebugger.WriteBreakPoint(filename, line, stack_info)
	IPCDebugger.GetDebugger().Write({filename="BP", type=debug_events_enum.BREAKPOINT, code = {filename=filename, line=line, stack_info=stack_info}});
end
...
function Handlers.setb(type, param1, param2, msg)
	IPCDebugger.SetBreakpointAsync(msg.filename, msg.line);
end

-- SetBreakpoint async: this is not recommended way to set breakpoint, call setb when breaked instead.
-- this is only used when we first attach a debugger to the running process to upload all break points at once. 
...
function IPCDebugger.GetDebugger()
	return IPCDebugger.debugger or IPCDebugger;
end

function IPCDebugger.SetDebugger(debugger)
	IPCDebugger.debugger = debugger;
end
...
local function debugger_loop(ev, vars, file, line, idx_watch, stack_info)
	...
	while true do
	write("[DEBUG]> ")
	local msg_in = IPCDebugger.GetDebugger().WaitForDebugEvent();
	...
	elseif command == 'exec' then
	  -- exec code in current location
		...
		if func == nil then
			IPCDebugger.Dump("Compile error: "..tostring(line)..'\n')
		elseif not ok then
			IPCDebugger.Dump("Compile error: "..tostring(func)..'\n')
		else
			...
			if res[1] then
			  ...
			  IPCDebugger.Dump("Run error: "..tostring(res[2])..'\n')
			end
		end
	end
	end
...
function IPCDebugger.Attach()
	...
	IPCDebugger.GetDebugger().Write({filename="Attached", type=debug_events_enum.ATTACHED, code = {
...
function IPCDebugger.Detach()
	...
	IPCDebugger.GetDebugger().Write({filename="Detach"});
end
