...
if(ls) then
	ls:GetURL(System.localserver.CachePolicy:new("access plus 1 week"),
...
-- @param Cache_policy: nil or System.localserver.CachePolicy, or string like "access plus 1 week"
...
function URLResourceStore:GetURL(Cache_policy, url, callbackFunc, callbackContext, funcMsgTranslator, bUseExpire)
	...
	if(Cache_policy.ExpireTime < 47959200) then
		local request = System.localserver.TaskManager:new_request(url, 1)
		if(request) then
			-- add message
