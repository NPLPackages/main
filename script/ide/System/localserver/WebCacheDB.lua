...
local webDB = System.localserver.WebCacheDB.GetDB()
-------------------------------------------------------
]]
...
NPL.load("(gl)script/ide/System/localserver/http_constants.lua");
-- partial definition of WebCacheDB
NPL.load("(gl)script/ide/System/localserver/WebCacheDB_def.lua");
NPL.load("(gl)script/ide/System/localserver/WebCacheDB_store.lua");
NPL.load("(gl)script/ide/System/localserver/WebCacheDB_permissions.lua");
	...
	for row in stmt:rows() do
		...
		if(is_cookie_required) then
			if(cookie_map == nil) then
				cookie_map = System.localserver.CookieMap:new(url);
			end
...
-- @param security_origin: type of System.localserver.SecurityOrigin or an URL string
...
function WebCacheDB:InsertServer(server)
	...
	if (not System.localserver.UrlHelper.IsStringValidPathComponent(server.name)) then
