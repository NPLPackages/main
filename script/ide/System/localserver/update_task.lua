...
function System.localserver.IsUpdateTaskForStoreRunning(store_id)
	-- TODO: 
end
...
function UpdateTask:Run(index)
	...
	if(i<=num_urls) then
		...
		if(System.localserver.UrlHelper.IsWebSerivce(url)) then
			...
			NPL.RegisterWSCallBack(wsAddr, string.format("System.localserver.ProcessWS_result(%d, %d)", self.capture_request_.id, i));
