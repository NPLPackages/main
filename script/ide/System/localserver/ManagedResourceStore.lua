...
------------------------------------------
local ManagedResourceStore = commonlib.inherit(System.localserver.localserver, {
	-- type of WebCacheDB.ServerType
	server_type_ = WebCacheDB.ServerType.MANAGED_RESOURCE_STORE,

	-- default policy
	Cache_policy = System.localserver.CachePolicy:new("access plus 1 month"),
});

commonlib.setfield("System.localserver.ManagedResourceStore", ManagedResourceStore);
...
function ManagedResourceStore:GetUpdateInfo() 
	...
	if (server) then
		...
		if ((server.update_status == WebCacheDB.UpdateStatus.UPDATE_CHECKING or
			...
			not System.localserver.IsUpdateTaskForStoreRunning(self.server_id_)) then
