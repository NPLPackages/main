...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/ide/System/localserver/test/test_webservice_store.lua");
test.test_callXML()
...
function test.test_WebserviceStore()
	...
	local ls = System.localserver.CreateStore("ws", 2);
	if(not ls) then
	...
	for i=1,2 do
		ls:CallWebserviceEx(System.localserver.CachePolicy:new("access plus 1 hour"),
	...
	ls:CallWebserviceEx(System.localserver.CachePolicy:new("access plus 0"),
			"http://www.kids3dmovie.com/CheckVersion.asmx", 
			-- msg
			{aaa="bbb"},
			{"aaa"},
...
function test.test_callXML(input)
	...
	ls:CallXML(System.localserver.CachePolicy:new(input.cache_policy or "access plus 10 minute"), input.url, function (xmlRoot, entry)
		log("HTML page retrieved\n")
		commonlib.log(entry)
	end)
	
	--ls:GetFile(System.localserver.CachePolicy:new("access plus 2 minute"), "http://www.paraengine.com/en/business.html", function (entry)
		--commonlib.log(entry)
		--log("Last modified: "..entry.payload:GetHeader(System.localserver.HttpConstants.kLastModifiedHeader))
		--log("Content length: "..entry.payload:GetHeader(System.localserver.HttpConstants.kContentLengthHeader))
	--end)
end
...
function test.localserver_GetURL(input)
	...
	if(ls) then
		ls:GetURL(input.cache_policy or System.localserver.CachePolicy:new("access plus 1 day"),
