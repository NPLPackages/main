...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/ide/System/localserver/test/test_resource_store.lua");
test.test_GetFile()
...
function test.test_ResourceStore()
	...
	local ls = System.localserver.CreateStore("AnotherStore");
	if(not ls) then
		log("error: failed creating local server resource store\n")
		return 
	else
...
function test.test_GetFile(input)
	...
	local ls = System.localserver.CreateStore("rs", 1);
	if(not ls) then
