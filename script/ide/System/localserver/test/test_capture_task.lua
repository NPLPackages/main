...
use the lib:
------------------------------------------------------------
NPL.load("(gl)script/ide/System/localserver/test/test_capture_task.lua");
...
function test.test_WS_CaptureTask()
	...
	local ls = System.localserver.CreateStore("MyStore");
	if(not ls) then
	...
	local request = System.localserver.TaskManager:new_request("http://www.kids3dmovie.com/CheckVersion.asmx")
	--local request = System.localserver.TaskManager:new_request("http://202.104.149.47/AuthUser.asmx?aaa=bbb")
	if(request) then
		-- add message
		request.msg = {aaa="bbb"};
		...
		local task = System.localserver.CaptureTask:new(ls, request);
		if(task) then
			task:Run();
		end
	end
	...
	local request = System.localserver.TaskManager:new_request({
