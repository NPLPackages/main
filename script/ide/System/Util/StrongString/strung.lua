--[[
Title: Strong string lib (Need luajit, lua51 are not supported)
Author(s): ported by LiXizhi from https://github.com/pygy/strung.lua
...
Date: 2023/2/11
Desc: lua 5.1 and luajit's string lib does not support \0 null char in both source and pattern. The bug is fixed in lua 5.3 though. 
Since we stick to luajit and lua51, we need a strong string class that support \0. 

use the lib:
------------------------------------------------------------
local strung = NPL.load("(gl)script/ide/System/Util/StrongString/strung.lua");
if(strung) then
    echo(strung.match("a\0b", "\0b") == "\0b")
    echo(strung.gsub("a\0b", "\0", "") == "ab")
end
------------------------------------------------------------
]]
--- localize globals ---
  ...
  while i <= last do
    ...
    if c == '%' then
      ...
      elseif s_sub(pat, i, i) == 'z'then 
		bitset(cs, 0); i = i + 1; bContinue = true
...
local gsub do
  ...
  local function malloc(s)
    local p = ffi.new('char[?]', s)
    if p == nil then error"out of memory, `ffi.new()` failed" end
    return p
  end
